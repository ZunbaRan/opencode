...
- upstream baseline: immutable official stable Release tag `v1.18.16`
- product branch: `openchamber-apps`
- release version: `<upstream-version>-oc.<revision>`
- next fork release: `1.18.16-oc.1` (not published yet)
- binary identity and user data paths remain `opencode`

Upstream updates are merged from immutable official stable Release tags with a
merge commit. Moving branches such as `dev` are never release inputs. Published
history is never rebased. A release carries the CLI for every supported
platform, generated SDK tarball, SHA256 manifest, upstream Release tag,
upstream/fork revisions, and GitHub provenance.
